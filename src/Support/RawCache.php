...
class RawCache
{
    ...
    {
        return (bool) ($this->config['raw_cache']['enabled'] ?? true);
    }

    public function getAgendaHtml(string $site, string $committeeName, string $date): ?string
    {
        $path = OutputPaths::rawAgendaHtmlPath($this->config, $site, $committeeName, $date);
    ...
    {
        $this->disk()->put(OutputPaths::rawAgendaHtmlPath($this->config, $site, $committeeName, $date), $html);
    }
    ...
    /**
     * Copy the cached attachment bytes to a real filesystem path so PDF
     ...
     * writing anything if the file isn't cached.
     */
    ...
    {
        return Storage::disk($this->config['raw_cache']['disk'] ?? ($this->config['output']['disk'] ?? 'local'));
    }

    protected function manifestPath(string $site, string $committeeName, string $date): string
    {
        return OutputPaths::rawAttachmentsPath($this->config, $site, $committeeName, $date).'/manifest.json';
    }

    protected function attachmentFilePath(string $site, string $committeeName, string $date, string $bookmark): string
    {
        return OutputPaths::rawAttachmentsPath($this->config, $site, $committeeName, $date).'/'.$bookmark;
    }
}
